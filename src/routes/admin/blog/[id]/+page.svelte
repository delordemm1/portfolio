		...
		<div class="px-4 py-6 sm:px-0">
			...
			<div class="bg-white shadow sm:rounded-lg mb-6">
				<div class="px-4 py-5 sm:p-6">
					<h2 class="text-lg font-medium text-gray-900 mb-4">Blog Post Settings</h2>
					<form 
						method="post" 
						action="?/update" 
					...
					<div class="flex items-center justify-between mb-6">
						<div>
							...
							<p class="text-sm text-gray-500">
								Build your blog post content using different types of blocks. Drag to reorder.
							</p>
						</div>
							...
							{#each data.blocks as block, index}
								...
								>
									...
									{#if editingBlock === block.id}
										<!-- Edit Mode -->
										<form method="post" action="?/updateBlock" enctype="multipart/form-data" use:enhance>
											<input type="hidden" name="blockId" value={block.id} />
												...
												<div class="space-y-4">
													{#if content.url}
														<div>
															...
															<img src={content.url} alt={content.alt} class="h-32 w-auto rounded-lg" />
														</div>
													{/if}
													<div>
														<label class="block text-sm font-medium text-gray-700 mb-2">
									...
									{:else}
										...
										{#if block.type === 'text'}
											<div class="prose max-w-none">
												{#if content.content}
													<div class="whitespace-pre-wrap text-gray-700">{content.content}</div>
												{:else}
													<div class="text-gray-400 italic">Empty text block - click Edit to add content</div>
												{/if}
											</div>
										{:else if block.type === 'image'}
											{#if content.url}
												<div>
													<img src={content.url} alt={content.alt || ''} class="max-w-full h-auto rounded-lg" />
													{#if content.caption}
														<p class="text-sm text-gray-600 mt-2 italic">{content.caption}</p>
													{/if}
												</div>
											{:else}
												<div class="text-gray-400 italic">No image uploaded - click Edit to add an image</div>
											{/if}
										{:else if block.type === 'youtube'}
											{#if content.url}
												...
												{#if embedUrl}
													<div>
														{#if content.title}
															<h4 class="text-lg font-medium text-gray-900 mb-2">{content.title}</h4>
														{/if}
												...
												{:else}
													<div class="text-red-400 italic">Invalid YouTube URL - click Edit to fix</div>
												{/if}
											{:else}
												<div class="text-gray-400 italic">No YouTube URL - click Edit to add a video</div>
											{/if}
										{:else if block.type === 'link'}
											{#if content.url}
												<div class="border border-gray-200 rounded-lg p-4 hover:border-gray-300 transition-colors">
													<a href={content.url} target="_blank" rel="noopener noreferrer" class="block">
														{#if content.title}
															<h4 class="text-lg font-medium text-blue-600 hover:text-blue-800">{content.title}</h4>
														{/if}
														{#if content.description}
															<p class="text-gray-600 mt-1">{content.description}</p>
														{/if}
														<p class="text-sm text-gray-500 mt-2">{content.url}</p>
													</a>
												</div>
											{:else}
												<div class="text-gray-400 italic">No URL provided - click Edit to add a link</div>
											{/if}
										{/if}
									{/if}
								</div>
